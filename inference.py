...
def inference(config):
    ...
    for i, (filenames, images) in enumerate(dataset_loader):
        try:
            ...
            outputs_mask_inv = torch.logical_not(outputs_mask.bool()).long()
...
if __name__ == '__main__':
    # parameter settings
    start_time = time.time()  # record beginning time
    ...
    elapsed_time = end_time - start_time  # inference time

    print(f"Inference time: {elapsed_time:.6f} s")
