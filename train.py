...
def train(config):
    ...
    model, classifier = model_choice(config.model_name, out_channels=2)
    ...
    lr_scheduler_1 = torch.optim.lr_scheduler.ExponentialLR(opt1, gamma=0.98) #opt1 is for seg
    lr_scheduler_2 = torch.optim.lr_scheduler.ExponentialLR(opt2, gamma=0.9)
    CELoss = nn.CrossEntropyLoss() # is not used in our model, loss functions are defined in Loss_Function.py

    print('running on {}'.format(config.device))

    # set visdom, visualization of training process (training loss, eval loss, learning rate)
    ...
    for epoch in range(config.model_prefix, config.epochs):
        ...
        for phase in ['train', 'eval']:
            ...
            for i, (images, labels) in enumerate(dataset_loaders[phase]):
                ...
                labels = labels.float()
                labels_one_hot = F.one_hot(labels.long(), num_classes=2)  # Shape: [batch_size, 512, 512, 2]
                labels_one_hot = labels_one_hot.permute(0, 3, 1, 2).float()  # Change to [batch_size, 2, 512, 512]
...
# ================================================================================= #
                    if config.segmentation_only:
                        # loss = CELoss(outputs, labels_one_hot)
                        loss = LossFunction(labels_one_hot,outputs)
# ================================================================================= #
                    else:
        ...
        # Focal+Dice is almost over 0.05, and over epoch 150, the training loss has been stable
        if epoch>150 and current_loss < config.min_loss_threshold:
            print(f"Loss {current_loss} is below the threshold {config.min_loss_threshold}. Stopping training.")
            return
